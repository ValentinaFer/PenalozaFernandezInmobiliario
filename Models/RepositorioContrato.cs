...
public class RepositorioContrato
{
    ...
    public IList<Contrato> GetAllForIndex(int pageSize, int pageNumber)
    {
        ...
        using (var connection = new MySqlConnection(ConnectionString))
        {
            ...
            p.Id, p.{nameof(Propietario.Apellido)}, p.{nameof(Propietario.Nombre)}, p.{nameof(Propietario.Dni)},
            inm.{nameof(Inmueble.Id)}, inm.{nameof(Inmueble.Direccion)}
            FROM Contratos c
            INNER JOIN Inmuebles inm ON c.{nameof(Contrato.InmuebleId)} = inm.{nameof(Inmueble.Id)}
            INNER JOIN Propietarios p ON inm.{nameof(Inmueble.PropietarioId)} = p.Id
